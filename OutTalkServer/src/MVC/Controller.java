...
public class Controller implements Observer {
	...
	public void initServerFunctionality(SocketIOServer serverSock) {
			...
			public void onData(SocketIOClient client, String data, AckRequest ackRequest) {
				...
				executionPool.execute(new Runnable() {
					...
					public void run() {
						...
						if (getClientEmailBySocket(client) != null && isEmailsEquals(getClientEmailBySocket(client), lrd.getUserEmail()))
						{
							...
							if (getClientEmailBySocket(client) != null
									&& !isEmailsEquals(getClientEmailBySocket(client), lrd.getUserEmail()))
							...
							if (connections.get(user.getEmail()) == null) {
								...
								if (isEmailsEquals(lrd.getUserEmail(), credential.getUser().getEmail())
		...
		serverSock.addEventListener("Request", String.class, new DataListener<String>() {
			...
			public void onData(SocketIOClient client, String data, AckRequest ackRequest) {
				...
				if (isEmailsEquals(getClientEmailBySocket(client), rd.getUserEmail())) {
					executionPool.execute(new Runnable() {
	...
	private void sendEventInventationToUsers(Event e, List<UserData> participants) {
		participants.forEach(p -> {
			...
			if (sock != null && !isEmailsEquals(e.getAdmin().getEmail(), p.getEmail())) {
				ClientHandler.sendToClient(sock, "Notification",
						new EventInvitationNotificationData(model.getDbManager().getEventDataByEvent(e, participants)));
			}
		});
	}
	...
	private void sendUserEventNotification(Event event, User user, Boolean isJoin) {
		...
		list.forEach(u -> {
			...
			if (sock != null && !isEmailsEquals(user.getEmail(), u.getEmail())) {
				ClientHandler.sendToClient(sock, "Notification",
	...
	public boolean isEmailsEquals(String mail1, String mail2) {
